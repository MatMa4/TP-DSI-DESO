...
public interface repositorioHuesped extends JpaRepository<Huesped, HuespedId> {
    ...
    @Query("SELECT h FROM Huesped h " +
           "WHERE (:nombre IS NULL OR h.nombre = :nombre) " +
           "AND (:apellido IS NULL OR h.apellido = :apellido) " +
           "AND (:tipoDocumento IS NULL OR h.id.tipoDocumento = :tipoDocumento) " +
           "AND (:numeroDocumento IS NULL OR h.id.numeroDocumento = :numeroDocumento)")
